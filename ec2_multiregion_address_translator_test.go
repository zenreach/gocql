...
func TestEC2MultiRegionAddressTranslator_NoHostsFound_ReturnsProvidedArgs(t *testing.T) {
	host, port := EC2MultiRegionAddressTranslator().Translate(net.ParseIP("10.10.220.255"), 9042)
	assertEqual(t, "ec2 translated host", "10.10.220.255", host.String())
	assertEqual(t, "ec2 translated port", 9042, port)
}

func TestEC2MultiRegionAddressTranslator_Translates(t *testing.T) {
	host, port := EC2MultiRegionAddressTranslator().Translate(net.ParseIP("10.10.220.254"), 9042)
	assertEqual(t, "ec2 translated host", "10.10.220.253", host.String())
	assertEqual(t, "ec2 translated port", 9042, port)
}
